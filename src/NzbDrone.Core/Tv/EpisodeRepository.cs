...
namespace NzbDrone.Core.Tv
{
    ...
    public class EpisodeRepository : BasicRepository<Episode>, IEpisodeRepository
    {
        ...
        private SqlBuilder EpisodesWhereCutoffUnmetBuilder(List<QualitiesBelowCutoff> qualitiesBelowCutoff, List<LanguagesBelowCutoff> languagesBelowCutoff, int startingSeasonNumber) => Builder()
            ...
            .Having(BuildQualityCutoffWhereClause(languagesBelowCutoff));
        ...
        private string BuildQualityCutoffWhereClause(List<QualitiesBelowCutoff> qualitiesBelowCutoff)
        {
            ...
            foreach (var profile in qualitiesBelowCutoff)
            {
                ...
                {
                    clauses.Add(string.Format("(Series.[QualityProfileId] = {0} AND MIN(EpisodeFiles.Quality) LIKE '%_quality_: {1},%')", profile.ProfileId, belowCutoff));
                }
            }
        ...
        private string BuildQualityCutoffWhereClause(List<LanguagesBelowCutoff> languagesBelowCutoff)
        {
            var clauses = new List<string>();
            ...
            foreach (var profile in languagesBelowCutoff)
            {
                ...
                {
                    clauses.Add(string.Format("(Series.[LanguageProfileId] = {0} AND MIN(EpisodeFiles.Language) LIKE '%_language_: {1},%')", profile.ProfileId, belowCutoff));
                }
            }
