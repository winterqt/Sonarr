...
namespace NzbDrone.Core.SeriesStats
{
    ...
    public class SeriesStatisticsRepository : ISeriesStatisticsRepository
    {
        ...
        private SqlBuilder Builder(DateTime currentDate) => new SqlBuilder()
            .Select(@"Episodes.SeriesId AS SeriesId,
                             ...
                             MAX(CASE WHEN AirDateUtc >= @currentDate OR EpisodeFileId = 0 AND Monitored = 0 THEN NULL ELSE AirDateUtc END) AS PreviousAiringString", new { currentDate = currentDate })
            .LeftJoin<Episode, EpisodeFile>((t, f) => t.EpisodeFileId == f.Id)
            .Where<Episode>(x => x.Monitored == true)
